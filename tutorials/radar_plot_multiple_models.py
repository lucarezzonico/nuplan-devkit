...
class CLStats:
    ...
    drivable_area_compliance: IdxUnit
    corners_in_drivable_area: IdxUnit
    abs_ego_lon_jerk_within_bounds: IdxUnit
    abs_ego_lon_acceleration_within_bounds: IdxUnit
    time_to_collision_within_bound: IdxUnit
    speed_limit_compliance: IdxUnit
    abs_ego_jerk_within_bounds: IdxUnit
    ego_expert_progress_along_route_ratio: IdxUnit # unit: ratio # normalize
    no_ego_at_fault_collisions: IdxUnit
    abs_ego_lat_acceleration_within_bounds: IdxUnit
    ego_is_making_progress: IdxUnit
    abs_ego_yaw_acceleration_within_bounds: IdxUnit
    ego_lane_change_fail_rate_below_threshold: IdxUnit
    abs_ego_yaw_rate_within_bounds: IdxUnit
    ego_is_comfortable: IdxUnit
    all: IdxUnit
...
# def call_metric_summary_callback():
#     cfg = yaml.safe_load(open("tutorials/radar_plot_multiple_models.yaml"))
    
#     output_dir = cfg["multiple_models"]["output_dir"]
    
#     for model_dir in cfg["multiple_models"]["models_dirs"]:
        
#         cfg["metric_summary_callback"]["metric_save_path"] = f"{output_dir}/{model_dir}/metrics"              # Path to saved metric files
#         cfg["metric_summary_callback"]["metric_aggregator_save_path"] = f"{output_dir}/{model_dir}/aggregator_metric"     # Path to saved aggregated files
#         cfg["metric_summary_callback"]["summary_output_path"] = f"{output_dir}/summary"
    
#         msc = MetricSummaryCallback(**cfg["metric_summary_callback"])
    
#         model_radar_plot_data = msc._get_model_radar_data()
        
#         model_radar_plot_data
...
if __name__ == '__main__':
    ...
    for model_dir in cfg["multiple_models"]["models_dirs"]:
        ...
        [radar_plot_keys_selected_statistics, radar_plot_values_selected_statistics,
         scenario_types_keys, scenario_types_values, scenario_types_keys, scenario_types_values_2], \
    ...
    matplotlib_plots = msc._render_radar_plot(models_radar_plot_data, simulation_type, model_names)
        
    msc._save_to_pdf(matplotlib_plots)
    msc._save_to_png(matplotlib_plots)
    
    plt.close('all')  # This will close all open figures
        
    # call_metric_summary_callback()
