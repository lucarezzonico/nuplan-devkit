...
class ClosedLoopTrajectoryWeightDecayImitationObjective(AbstractObjective):
    """
    ...
    def __init__(self, scenario_type_loss_weighting: Dict[str, float], weight: float = 1.0):
        ...
        """
        self._name = 'closed_loop_trajectory_weight_decay_imitation_objective'
        self._weight = weight
        self._decay = 1.0

        self._fn_xy = torch.nn.modules.loss.L1Loss(reduction='none')
        self._fn_heading = torch.nn.modules.loss.L1Loss(reduction='none')
        self._fn_v = torch.nn.modules.loss.MSELoss(reduction='none')
